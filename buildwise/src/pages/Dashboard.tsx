...
import { FolderKanban, ListTodo, Briefcase, TrendingUp, AlertCircle, Loader2, BrainCircuit } from "lucide-react";
import { Link } from "wouter";
import { useAuth } from "@/context/AuthContext";
import { isGeminiConfigured } from "@/lib/gemini-ai-advisor";
import { canUseAiAdvisor, canViewVendors, canWorkBoard, softwareRoleLabel } from "@/lib/software-roles";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-primary/10 to-indigo-500/10 p-6 rounded-2xl border border-white/5 relative overflow-hidden">
        ...
        <div className="z-10">
          <p className="text-xs uppercase tracking-[0.24em] text-indigo-300">Software portal · {roleLabel}</p>
          <h2 className="text-2xl font-bold font-display text-white mt-1">Welcome back, {firstName}</h2>
          <p className="text-slate-400 mt-1">Building web, desktop, mobile, and enterprise products. Issuer meetings live in Governance.</p>
        </div>
        {showAi && (
        <Link href="/ai-advisor" className="z-10">
          <Button className="bg-indigo-600 hover:bg-indigo-500 shadow-indigo-500/25 border border-indigo-400/30">
            {aiReady ? <TrendingUp className="w-4 h-4 mr-2" /> : <BrainCircuit className="w-4 h-4 mr-2" />}
            {aiReady ? "View AI Insights" : "Open AI Advisor"}
          </Button>
        </Link>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Link href="/projects" className="block group">
          <Card className="bg-gradient-to-br from-card to-card border-white/5 h-full group-hover:border-primary/40 transition-colors">
            <CardContent className="p-6 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center text-blue-500">
                <FolderKanban className="w-6 h-6" />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-400">Software products</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Software products by status</CardTitle>
          </CardHeader>
          ...
          <CardContent className="h-[300px]">
            {stats.tasksByStatus.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-2">
                ...
                <p>No tasks yet.</p>
                <Link href="/board" className="text-sm text-primary hover:underline">Open the board to add work</Link>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
